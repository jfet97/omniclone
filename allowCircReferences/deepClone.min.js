...
res[prop]=realDeepCopy(value,{setPrototype,invokeConstructors,copyNonEnumerables,copySymbols,copyGettersSetters,allowCircularReferences,},references,start);safeReferences.set(value,res[prop])}else{Object.defineProperty(res,prop,descriptor)}});if(allowCircularReferences){references.set(source,res);if(start==source){(function updateReferences(res,references){Object.entries(res).forEach(([key,value])=>{if(value&&typeof value=='object'){if(references.has(value)){res[key]=references.get(value)}else{updateReferences(res[key],references)}}})})(res,references)}}
